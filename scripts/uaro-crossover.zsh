...
usage() {
  print "Usage: uaro-crossover.zsh <preflight|continue|artifact|bottle|install|patch-setup|check-gecko|configure|configure-keyboard|deploy-app|overlay|build-launchers|verify-registration|launch-patcher|launch-setup|verify-live-runtime|diagnose|repair|uninstall> [options]"
  print ""
...
case "$command" in
  preflight)
    ...
    ;;
  continue)
    exec python3 "$SCRIPT_DIR/continue.py" "$@"
    ;;
  artifact)
