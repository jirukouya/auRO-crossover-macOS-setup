...
First inspect the live launch route. If the process uses stock or mixed CrossOver paths, close it and relaunch with `UaRO CrossOver Patcher.app`. If the route is overlay, compare the loaded DLL hash with the artifact manifest and inspect stock/after probe logs. Do not replace uaRO game DLLs based on this T-code alone.
