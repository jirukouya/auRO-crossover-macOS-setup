...
- The normal route now fetches the exact build-matched `gepard-crossover-fix-<build>-discord.zip` Release asset from `jirukouya/auRO-crossover-macOS-setup`. The archive digest and inner `SHA256SUMS` are verified before import; the community prebuilt's source revision, binary signature, and redistribution license remain unconfirmed. A manually supplied Discord folder is only an explicit fallback.
